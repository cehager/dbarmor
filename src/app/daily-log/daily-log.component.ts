...
export class DailyLogComponent implements OnInit, AfterViewInit {
  ...
  onRowSelected(event) {
    ...
    this.currentDailyLog.logDate = moment(event.data.logDate).format(
      "MM/DD/YYYY"
    );
  ...
  ngOnInit() {
    ...
    $.FroalaEditor.RegisterCommand("alert", {
      ...
      callback: function () {
        alert(
          "This is a future feature that will cause highlighted text to be blacked out!"
        );
      },
    });
    ...
    this.items = [
      {
        label: "Create New Log/Diary Entry",
        icon: "fa-refresh",
        command: () => {
  ...
  btnDelete() {
    this.dialogVisible = false;
    this.appRepository
      .deleteDailyLog(this.currentDailyLog.id)
      .subscribe((data) => {
  ...
  doEncrypt() {
    ...
    if (rmsg.length === 0) {
      ...
      this.alertify.dialog(
        "Got It!",
        "<h4>Ooops! There is nothing to encrypt!<h4>"
      );
      return;
    }
    ...
    if (!m.isValid() || this.logDate === "undefined" || this.logDate === null) {
      this.alertify.dialog(
        "Got It!",
        "Please enter a valid date. (Today's date has been inserted for your convenience)."
      );
      this.logDate = moment().format("MM/DD/YYYY");
      return;
    }
    ...
    this.appRepository
      .doEncryptDailyLog(this.currentDailyLog, this.apiPath)
      .subscribe(
        (data: DailyLog) => {
          ...
          this.appRepository
            ...
            .subscribe(
              (data: DailyLog[]) => {
                this.dailyLogs = data;
                for (let i = 0; i < this.dailyLogs.length; i++) {
                  this.dailyLogs[i].logDate = moment(
                    this.dailyLogs[i].logDate
                  ).format("MM/DD/YYYY");
                }
  ...
  doDecrypt() {
    ...
    this.appRepository
      .doDecryptDailyLog(this.currentDailyLog, this.apiPath)
      .subscribe(
        (data: DailyLog) => {
